...
export class AnalyticsService {
  ...
  public getAnalytics(favoriteObject: any, favoriteType: string, customFilter: any[] = [], externalSource: boolean = false): Observable<any> {
    const url = externalSource ? this._constructAnalyticUrlForExternalSource(favoriteObject) : this._constructAnalyticUrl(favoriteObject, favoriteType, customFilter);
  ...
  private _constructAnalyticUrl(favoriteObject: any, favoriteType: string, customFilter: any[]): string {
     ...
     * @type {string}
     */
    const rowParameters = this._getDimension('rows', favoriteObject, customFilter);
    const columnParameters = this._getDimension('columns', favoriteObject, customFilter);
    const filterParameters = this._getDimension('filters', favoriteObject, customFilter);
      ...
      favoriteObject[dimension].forEach((dimensionValue: any) => {
        ...
        if (dimensionValue.hasOwnProperty('dimension') && dimensionValue.dimension !== 'dy') {
          ...
          if (dimensionValue.hasOwnProperty('items') && dimensionValue.items.length > 0) {
             ...
             * @type {string}
             */
            const customDimensionValue = this._getCustomDimensionValue(customFilter, dimensionValue.dimension);
