...
export class DataFilterComponent implements OnInit {
  ...
  toggleDataOption(optionPrefix, event) {
    const someItems = this.manyItemsSelected(this.dataOptions, optionPrefix);
    if (event.ctrlKey) {
  ...
  dataItemList() {
    ...
    if (_.includes(selectedOptions, 'ALL') || _.includes(selectedOptions, 'de')) {
      if (group.id === 'ALL') {
        ...
        if (group.hasOwnProperty('dataElements')) {
          const newArray = _.filter(data.dx, (dataElement) => {
      ...
      currentList = _.map(currentList, (item: any) => {
        return {...item, type: 'Data Element'}
      })
    }
    ...
    if (_.includes(selectedOptions, 'ALL') || _.includes(selectedOptions, 'in')) {
      if (group.id === 'ALL') {
        ...
        if (group.hasOwnProperty('indicators')) {
          const newArray = _.filter(data.ind, (indicator) => {
    ...
    if (_.includes(selectedOptions, 'ALL') || _.includes(selectedOptions, 'cv')) {
      if (group.id === 'ALL') {
        this.metaData.dataSetGroups.forEach((group1) => {
          currentList.push(...data.dt.map(datacv => {
            const dataSet: any = {id: datacv.id + group1.id, name: group1.name + ' ' + datacv.name};
  ...
  // selecting all items
  selectAllItems() {
    const newList = this.filterByName.transform(this.listItems, this.listchanges);
  ...
  getSelectedPeriods() {
    const periods = [];
    for (const data_item of this.selectedItems) {
      if (data_item.hasOwnProperty('dataSets')) {
        for (const dataset of data_item.dataSets) {
      ...
      if (data_item.hasOwnProperty('dataSetElements')) {
        for (const dataset of data_item.dataSetElements) {
