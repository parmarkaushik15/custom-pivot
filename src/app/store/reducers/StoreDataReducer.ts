...
import {
  ...
  ADD_SINGLE_EMPTY_ANALYITICS, SET_ORGANISATION_MODEL_ACTION, SET_PERIOD_TYPE_ACTION,
  SET_YEAR_ACTION, ADD_FUNCTION_MAPPING_ACTION, ADD_FUNCTIONS_ACTION, ADD_SINGLE_AUTOGROWING_ANALYITICS,
  UPDATE_TABLE_ACTION, RESET_TABLE_OBJECTS_ACTION, UPDATE_CURRENT_ANALYTICS_OPTIONS_ACTION, UPDATE_OPTIONS_ACTION, BaseActions,
} from '../actions';
...
function handleAddSingleDataAnalyticsAction(state: StoreData, action: any): StoreData {
  const newStore = _.cloneDeep(state);
  const analyticsExist = getIndexofAnalytics(newStore.dataAnalytics, action.payload.dataId);
  let add_item = true;
...
function handleAddSingleAutogrowingAnalyticsAction(state: StoreData, action: any): StoreData {
  const newStore = _.cloneDeep(state);
  const analyticsExist = getIndexofAnalytics(newStore.autoGrowingAnalytics, action.payload.dataId);
  let add_item = true;
  if (analyticsExist.checker) {
